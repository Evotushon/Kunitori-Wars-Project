...
public final class zzdvj implements zzdoy {
    private static final ThreadLocal<Cipher> zzhlj = new zzdvi();
    private static final ThreadLocal<Cipher> zzhlk = new zzdvl();
    private final SecretKeySpec zzhlg;
    private final byte[] zzhll;
    private final byte[] zzhlm;
    private final int zzhln;

    public zzdvj(byte[] bArr, int i) throws GeneralSecurityException {
        if (i == 12 || i == 16) {
            this.zzhln = i;
            zzdwv.zzez(bArr.length);
            this.zzhlg = new SecretKeySpec(bArr, "AES");
            Cipher cipher = zzhlj.get();
            cipher.init(1, this.zzhlg);
            this.zzhll = zzo(cipher.doFinal(new byte[16]));
            this.zzhlm = zzo(this.zzhll);
            return;
        }
    ...
    private final byte[] zza(Cipher cipher, int i, byte[] bArr, int i2, int i3) throws IllegalBlockSizeException, BadPaddingException {
        ...
        bArr3[15] = (byte) i;
        if (i3 == 0) {
            return cipher.doFinal(zzd(bArr3, this.zzhll));
        }
        ...
        if (copyOfRange.length == 16) {
            bArr2 = zzd(copyOfRange, this.zzhll);
        } else {
            byte[] copyOf = Arrays.copyOf(this.zzhlm, 16);
    ...
    public final byte[] zzc(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        if (length <= (Integer.MAX_VALUE - i) - 16) {
            ...
            System.arraycopy(zzey, 0, bArr4, 0, this.zzhln);
            Cipher cipher = zzhlj.get();
            cipher.init(1, this.zzhlg);
            ...
            Cipher cipher2 = zzhlk.get();
            cipher2.init(1, this.zzhlg, new IvParameterSpec(zza));
            cipher2.doFinal(bArr, 0, bArr3.length, bArr4, this.zzhln);
            byte[] zza3 = zza(cipher, 2, bArr4, this.zzhln, bArr3.length);
            int length2 = bArr3.length + this.zzhln;
