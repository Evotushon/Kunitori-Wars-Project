...
public final class zzdvy {
    ...
    public static ECParameterSpec zza(zzdwa zzdwa) throws NoSuchAlgorithmException {
        int i = zzdwb.zzhmi[zzdwa.ordinal()];
        if (i == 1) {
        ...
        String valueOf = String.valueOf(zzdwa);
    ...
    public static ECPublicKey zza(zzdwa zzdwa, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec zza = zza(zzdwa);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, zza.getCurve());
        return (ECPublicKey) zzdwf.zzhmx.zzhg("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator zzhg = zzdwf.zzhmw.zzhg("EC");
        zzhg.initialize(eCParameterSpec);
        return zzhg.generateKeyPair();
    }
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        PublicKey generatePublic = zzdwf.zzhmx.zzhg("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhg = zzdwf.zzhmv.zzhg("ECDH");
        zzhg.init(eCPrivateKey);
        try {
            zzhg.doPhase(generatePublic, true);
            byte[] generateSecret = zzhg.generateSecret();
