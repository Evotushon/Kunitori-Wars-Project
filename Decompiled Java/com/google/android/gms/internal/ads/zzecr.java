...
public final class zzecr {
    ...
    private zzecr(byte[] bArr, int i, int i2) {
    ...
    private zzecr(ByteBuffer byteBuffer) {
        this.zzakt = byteBuffer;
        this.zzakt.order(ByteOrder.LITTLE_ENDIAN);
    }

    public static zzecr zzz(byte[] bArr) {
        return zzq(bArr, 0, bArr.length);
    }

    public static zzecr zzq(byte[] bArr, int i, int i2) {
        return new zzecr(bArr, 0, i2);
    }
    ...
    public final void zzf(int i, String str) throws IOException {
        ...
        try {
            ...
            if (zzgh == zzgh(str.length() * 3)) {
                ...
                if (this.zzakt.remaining() >= zzgh) {
                    this.zzakt.position(position + zzgh);
                    zzd(str, this.zzakt);
                    int position2 = this.zzakt.position();
                    this.zzakt.position(position);
                    zzhc((position2 - position) - zzgh);
                    this.zzakt.position(position2);
                    return;
                }
                throw new zzecu(position + zzgh, this.zzakt.limit());
            }
            zzhc(zza(str));
            zzd(str, this.zzakt);
        } catch (BufferOverflowException e) {
            zzecu zzecu = new zzecu(this.zzakt.position(), this.zzakt.limit());
            zzecu.initCause(e);
            throw zzecu;
        }
    }

    public final void zza(int i, zzeda zzeda) throws IOException {
        zzab(i, 2);
        if (zzeda.zzhnu < 0) {
            zzeda.zzbda();
        }
    ...
    public final void zze(int i, zzeah zzeah) throws IOException {
        if (this.zzhzs == null) {
    ...
    public final void zza(int i, byte[] bArr) throws IOException {
        ...
        if (this.zzakt.remaining() >= length) {
            this.zzakt.put(bArr, 0, length);
            return;
        }
    ...
    public static int zzb(int i, zzeda zzeda) {
        int zzfz = zzfz(i);
        int zzbda = zzeda.zzbda();
        return zzfz + zzgh(zzbda) + zzbda;
    }
    ...
    public final void zzbcc() {
        if (this.zzakt.remaining() != 0) {
            throw new IllegalStateException(String.format("Did not write as much data as expected, %s bytes remaining.", new Object[]{Integer.valueOf(this.zzakt.remaining())}));
        }
    }
    ...
    public final void zzft(long j) throws IOException {
        while ((-128 & j) != 0) {
            zzhb((((int) j) & 127) | 128);
            j >>>= 7;
        }
