...
public final class zzdlg {
    ...
    public static <T> T zza(@NonNullDecl T t, @NullableDecl String str, @NullableDecl Object obj) {
        ...
        throw new NullPointerException(zzdlh.zzb(str, obj));
    }
    ...
    public static int zzs(int i, int i2) {
        ...
        if (i < 0) {
            str = zzdlh.zzb("%s (%s) must not be negative", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i));
        } else if (i2 < 0) {
            ...
            str = zzdlh.zzb("%s (%s) must be less than size (%s)", FirebaseAnalytics.Param.INDEX, Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    private static String zzb(int i, int i2, @NullableDecl String str) {
        if (i < 0) {
            return zzdlh.zzb("%s (%s) must not be negative", str, Integer.valueOf(i));
        } else if (i2 >= 0) {
            return zzdlh.zzb("%s (%s) must not be greater than size (%s)", str, Integer.valueOf(i), Integer.valueOf(i2));
        } else {
            StringBuilder sb = new StringBuilder(26);
            sb.append("negative size: ");
            sb.append(i2);
    ...
    public static void zzf(int i, int i2, int i3) {
        ...
        if (i < 0 || i2 < i || i2 > i3) {
            if (i < 0 || i > i3) {
                ...
                str = zzdlh.zzb("end index (%s) must not be less than start index (%s)", Integer.valueOf(i2), Integer.valueOf(i));
            }
            throw new IndexOutOfBoundsException(str);
        }
    }
