...
public final class zzclv {
    public static ArrayList<zzsz.zzj.zza> zzb(SQLiteDatabase sQLiteDatabase) {
        ArrayList<zzsz.zzj.zza> arrayList = new ArrayList<>();
        ...
        while (query.moveToNext()) {
            try {
                arrayList.add(zzsz.zzj.zza.zzg(query.getBlob(query.getColumnIndexOrThrow("serialized_proto_data"))));
            } catch (zzdzh e) {
                zzawf.zzey("Unable to deserialize proto from offline signals database:");
                zzawf.zzey(e.getMessage());
            }
        }
        query.close();
        return arrayList;
